...
The `token` argument is a trust boundary. The helper does not verify that an address implements honest token behavior. Select the token from trusted per-network server configuration or an allowlist, never unvalidated client input, and inspect the complete authorization tree before signing. Soroban binds the root arguments and every nested invocation requiring source authorization to the approval; changing the token afterward does not reuse that approval. However, approving a tree generated by a malicious token can intentionally authorize unintended transfers. See [Stellar's authorization rules](https://developers.stellar.org/docs/learn/fundamentals/contract-development/authorization).
...
- Replacing the approved token rejecting at the authorization boundary.
- An untrusted token's extra transfer rejecting without matching nested authorization.
- A deliberately overbroad approval allowing an untrusted token to move another asset, demonstrating why token selection and tree inspection matter.
...
Validation completed with Rust 1.91.0: all seven native tests passed, `cargo fmt --check` passed, and `cargo build --locked --target wasm32v1-none --release` produced the contract Wasm. The typed JSON request was also encoded against the compiled Wasm ABI, preserving both `i128` amounts and `Address` recipients. No contract was deployed and no network transaction was submitted. The recommended Stellar CLI build command above was documented from official instructions; it was not run locally.

Run the Python request tests with `python -m unittest discover -p 'test_*.py' -v`. They mock HTTP transport to verify timeout retries preserve the key and payload, new batches use their caller's key, missing or blank keys never send a request, and HTTP errors propagate. These tests do not exercise live API deduplication.
...
transaction = create_batch_transaction(
    ...
    idempotency_key=PERSISTED_BATCH_IDEMPOTENCY_KEY,
...
The required `idempotency_key` is sent as `x-idempotency-key`. Generate and persist a unique key with each logical payout job before the first request; reuse that key and unchanged payload after a timeout or connection failure. Never generate a fresh key inside a retry loop or reuse a key for a different job. Persist the returned transaction ID for approval and reconciliation. A timeout does not establish that creation failed, and retry errors do not prove the original transaction is absent.

The source needs sufficient spendable tokens, and recipients must satisfy the token's authorization rules. Simulation and network resource limits still apply. Reconcile uncertain results instead of submitting the same payout with a new key. Atomicity alone does not prevent duplicate batches.
